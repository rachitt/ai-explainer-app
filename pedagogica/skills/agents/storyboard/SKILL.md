...
- `hook_question`: copy `01_intake.json.hook_question` **verbatim**.
...
- The hook scene's `visual_intent` and `narration_intent` must directly set up `hook_question`. The hook beat is this question made concrete on screen.
- If there is no `hook` beat because the video is under 120 seconds, the **first** scene's `narration_intent` must reference `hook_question`.
...
## Pedagogy arc — mandatory structure

Every storyboard must walk the viewer through five stages, in this order. The arc is what turns a topic dump into teaching. Stage names are not beat_types; one beat can serve one stage, or a short beat can bridge two. But every stage must appear or be explicitly justified-away in the scene `narration_intent`.

| stage | what happens | maps to beat_type | ~fraction |
|---|---|---|---|
| **1. Hook** | Pose `hook_question` on screen with a concrete visual anchor. No jargon in the narration. | `hook` | 10–15% |
| **2. Concrete instance** | One specific example the viewer can hold in mind. Real numbers, real picture. Not the general principle yet. | `define` or `motivate` | 20–25% |
| **3. Struggle** | Invite the viewer to predict; reveal a surprise or gap. "What do you think happens next?" — then show the unexpected. This is the stage Khan Academy and 3Blue1Brown nail and textbook videos skip. | `motivate` or embedded in `define` | 10–15% |
| **4. Resolve** | Walk through the working. Name the principle *after* the viewer has wanted it. Symbols arrive attached to the thing they name. | `define` + `example` | 35–45% |
| **5. Generalize** | Zoom out: where else does this apply? Connect to what the viewer already knows. Cashes the hook_question in one sentence. | `generalize` + `recap` | 10–15% |

The Struggle stage is the non-obvious one. It means the storyboard MUST include a moment where the viewer is asked to predict, then shown they were (likely) wrong or that there is more going on. Concretely: one scene's `narration_intent` should be a variant of "what do you think happens if …?" or "before we work it out, guess …". The following scene reveals the answer.

Example arc for a 120s KVL video (the job just run):
- scene_01 hook: loop walker about to traverse a circuit; "why do voltages around a loop always add to zero?" → **Hook**
- scene_02 define: walker steps through, tally updates; concrete 9V / 3Ω / 6Ω → **Concrete + Resolve**
- scene_03 example: ask the viewer "what if we flip one resistor?" before showing it flips signs; reveal → **Struggle + Resolve**
- scene_04 recap: generalize "voltage-around-a-loop is a potential round trip; KVL = no free energy" → **Generalize**

Anti-examples:
- Five define scenes in a row. No struggle, no concrete anchor. Textbook, not teaching.
- Hook scene that names the principle in the first sentence. Kills curiosity before it starts.
- Generalize before struggle. Viewer has nothing to cash in.

If the video is <90s and can't fit all five, merge Concrete+Resolve, keep Hook + Struggle + Generalize explicit. Never skip Hook or Struggle.
