...
## 2026-04-23 — preflight area-threshold masked axis-vs-caption overlaps
**Mistake:** added a `_MARKER_AREA_THRESHOLD = 0.08` ignore rule to preflight so it would skip tick-mark overlaps. Because Lines have bbox area 0 (zero-width or zero-height), the threshold wiped out *all* Line-vs-X pairs — including real bugs like x-axis baseline vs ZONE_BOTTOM caption. Shipped projectile scene_05 with the axis baseline at y=-2.575 intersecting the caption at y=-2.75; preflight said "ok."
**Root cause:** area is the wrong metric for line-like primitives. `width × height = 0` doesn't mean "ignore."
**Fix:** removed `_MARKER_AREA_THRESHOLD` and `_bbox_area`. Kept the `isinstance(a|b, Dot)` ignore (dots really are intentional markers) and the plot_curve-vs-plot_curve ignore (intentional overlays). Added chalk-code SKILL rule: axes bbox (including tick overhang) must leave ZONE_BOTTOM clear when a ZONE_BOTTOM caption is present. Concrete formula: `cy - height/2 - 0.15 ≥ -2.0`.
**Applies to:** every preflight tweak. Test against a known-bad scene before shipping; "no overlaps reported" on a buggy scene is a preflight regression, not a success.

## 2026-04-23 — our scripts lack Khan Academy's demo-run cadence
**Mistake:** projectile rerun opened scene_01 with "Throw a ball to a friend. It rises, slows, turns, and drops into their hands." User: "our script is trash right now." Compared against Khan Academy AP Physics projectile lecture (686s, 1970 words), we run 2 first-person markers vs KA's 86, 1 demo-action word vs 38, 17% short sentences vs KA's 30%. Our narrator *describes* the scenario; Sal Khan *performs* the demo.
**Root cause:** `script/SKILL.md` "Spoken-narration style" emphasized sentence length and LaTeX hygiene but didn't set quotas for short punchy lines, rhetorical questions, first-person plural ("we/let's"), or demo-action vocabulary ("ready/here goes/boom/watch/notice"). Ease-in rule said "open with a familiar phenomenon" but didn't require the 2–5 word question-hook and the demo-prediction-reveal cadence.
**Fix:** added "Cadence quotas" block to `script/SKILL.md` with concrete floors: ≥25% short (≤6-word) sentences, ≥1 question per 40 words, ≥8 first-person markers per 100 words, ≥2 demo-action words per scene. Rewrote the "Opening: ease the viewer in" section with the KA template (question hook → concrete demo → prediction invite → action cue → reveal → mystery). Added anti-patterns banning passive scenario-describing and zero-quota scripts.
**Applies to:** every script pass. Delegation briefs to the script agent must reference the Cadence quotas section and the KA 2026-04-23 benchmark. If a rewrite doesn't hit all four quotas, it's not finished.
